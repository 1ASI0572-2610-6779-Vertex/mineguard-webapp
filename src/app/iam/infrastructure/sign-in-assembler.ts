...
export class SignInAssembler {
   ...
   * a SignInResource that contains the authenticated user information, role,
   * and the authentication token for establishing the session.
   */
  toResourceFromResponse(response: SignInResponse): SignInResource {
    return {
      ...
      subscriptionPlan: response.subscriptionPlan ?? 'STANDARD',
    } as SignInResource;
  }
